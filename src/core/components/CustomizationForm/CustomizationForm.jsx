        ...
        {["Acabado", "Aleaciones", "Peso"].map((label, index) => (
          <div key={index} className="flex justify-center w-full mb-2 md:w-1/3">
            <select
              ...
              onChange={(e) => {
                if (label.toLowerCase() === "acabado") {
                  handleFinishChange(e.target.value);
                } else if (label.toLowerCase() === "aleaciones") {
                  handleAlloyChange(e.target.value);
                } else if (label.toLowerCase() === "peso") {
            ...
            >
              ...
              {label === "Peso" &&
                weightSizes?.map((opt) => (
                  <option key={opt.id} value={opt.id}>
                    {`${opt?.weight} (${opt?.diameter} cm x ${opt?.height}) cm  `}
                  </option>
                ))}
            </select>
          </div>
        ...
        <div className="flex flex-col lg:space-y-2 w-full lg:w-auto items-center">
          <div className="flex items-center">
            ...
            <p className="md:text-xl">
              {selectedSize ? selectedSize.weight : "No seleccionado"} kg
          ...
          <div className="flex items-center">
            ...
            <p className="md:text-xl">
              {selectedAlloy?.type || "No seleccionado"} - $
              {selectedAlloy?.pricePerKg}
            </p>
          </div>
        </div>
        <div className="flex flex-col lg:space-y-2 w-full lg:w-auto items-center">
          <div className="flex items-center">
            ...
            <p className="md:text-xl">
               {/* si aun no se seleciona poner un texto */}
               {selectedSize ? `${selectedSize.diameter} cm x ${selectedSize.height} cm` : "No seleccionado"}
          ...
          <div className="flex items-center">
            ...
            <p className="md:text-xl">
               {/* si aun no se seleciona poner un texto */}
              {selectedFinish ?  selectedFinish?.finish : "No seleccionado"}
