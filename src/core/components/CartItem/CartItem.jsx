...
const CartItem = ({ item, deleteItem, updateQuantity }) => {
      ...
      <div className="grid grid-cols-2 items-center justify-items-between w-full sm:flex sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col">
          <div className="relative flex items-center md:max-w-[8rem] mb-1">
            <button onClick={decrement} disabled={item.quantity <= 1} className={`bg-gray-100 border border-gray-300 rounded-s-lg p-3 h-11 ${item.quantity <= 1
            ...
            <input type="text" value={item.quantity} className="bg-gray-50 border-x-0 border-gray-300 h-11 text-center text-gray-900 text-sm w-full" readOnly />
          ...
          <button type="button" onClick={() => deleteItem(item.id)} className="flex items-center justify-center w-full font-medium rounded-full text-sm text-center bg-gray-200 hover:bg-gray-300 focus:ring focus:outline-none focus:ring-gray-400 py-2">
